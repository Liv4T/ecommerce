...
<div class="card" :id="value.id">
	<img :src="storageURL+value.product_image" style="height: 150px; width: 250px" :alt="value.product_name">
	<div style="padding-left: 15px">
		<h3 style="overflow: hidden;"><b>{{ minify(value.product_name) }}</b></h3>
		<h5>Precio : <span  style="color: lime;">{{ toCurrency(value.product_price) }}</span></h5>
		<h5>Stocks : {{ value.product_quantity }}</h5>
	</div>
	<div class="input-group card-btn" @click="addToCart();" v-if="cart_quantity==0">
		<button class="btn btn-info card-btn">Añadir al carrito</button>
	</div>
	<div v-if="cart_quantity>0">
		<div class="input-group">
			...
			<div class="form-control" style="text-align: center;">
				<b><span>{{ cart_quantity }}</span> items en el carro</b>
			</div>
			<div class="input-group-btn">
